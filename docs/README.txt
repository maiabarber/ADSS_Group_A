...
1.	System Overview
This document combines the user instructions for the Employee Scheduling module and the Delivery Management module. It also includes the initial database content inserted by DatabaseSeeder.seedSampleData().
•	The system is console-based. All actions are selected by typing the relevant menu number and pressing Enter.
•	The system supports HR manager actions, employee actions, and transportation manager actions.
...
Executable file:	adss2025_v01.jar
Run command:	java -jar adss2025_v01.jar
Recommended run steps: download the project ZIP, extract it, open Command Prompt or PowerShell in the release folder, and run the JAR command above. Use the terminal for all input.
...
This module is used for managing employees, roles, shifts, shift assignments, employee constraints, and HR-related actions.
...
After starting the delivery module, the main menu appears. Each action is selected by typing the option number and pressing Enter.
...
When creating a new delivery, the system asks for details such as delivery date, departure time, shipping zone, source site, truck, driver, stops, and items.
•	Date format: YYYY-MM-DD
•	Time format: HH:MM
•	If an invalid value is entered, the system displays an error and asks for the value again.

7.2 Recommended First Use
•	Start the system.
•	Choose option 2 - Load sample data.
•	Choose option 3 to view existing deliveries.
•	Choose options 4-7 to view sites, trucks, drivers, and shipping zones.
•	Create a new delivery or update an existing delivery as needed.

8.	 Initial Database Data
The following tables and rows are inserted by the DatabaseSeeder class using INSERT OR IGNORE statements. This means that if a row with the same primary key already exists, it will not be inserted again.
...
Row	  Values
1	    employee_id = 100000001; name = HR_MANAGER
2	    employee_id = 100000002; name = Store Keeper
3	    employee_id = 100000003; name = Driver Employee
...
9.	 Important Notes
•	Option 1 in the delivery module clears the transportation data from memory. Use it only when you want to work with an empty system.
