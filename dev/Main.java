...
public class Main {
        ...
        try (Scanner scanner = new Scanner(System.in)) {
            ...
            while (running) {
                ...
                if (isHrManager(loggedInUser.get())) {
                    switch (choice) {
                        case "1":
                            addNewEmployeeFlow(scanner, authenticationService, employeeRepository);
                            break;
                        case "2":
    ...
    private static boolean isHrManager(User user) {
        return user instanceof HR_Manager && ((HR_Manager) user).isHRManager();
    }

    private static void addNewEmployeeFlow(
        Scanner scanner,
    ...
    private static EmploymentType readEmploymentType(Scanner scanner) {
        while (true) {
    ...
    private static Set<Role> readAuthorizedRoles(Scanner scanner) {
